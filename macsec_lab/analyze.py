...
- [captures/decoded/13-mka-rekey.md](../captures/decoded/13-mka-rekey.md) — SAK 换钥：AN=0 → AN=1 全过程（`mka-rekey.pcap`）

格式与密钥体系背景：[第二章 密钥体系](../02_key_hierarchy/README.md)、[第五章 报文字节级解析](../05_wire_format/README.md)。
...
def write_reports(capture_dir: Path, out_dir: Path, docs_dir: Path | None = None) -> list[Path]:
    ...
    jobs = [
        ...
        ("mka-delay-protect.pcap", "delay", "20-mka-delay-protect.md"),
