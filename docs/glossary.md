...
| **802.1AE** | — | MACsec 数据面标准（SecY、SecTAG、GCM），2006 首版、2018 现行版（[standards-map.md](standards-map.md)） |
...
| **SecY** | MAC Security Entity | 数据面实体：protect/validate 帧的逻辑功能（[secy-processing.md](secy-processing.md)） |
...
## 2. 密钥体系（详见 [key-hierarchy.md](key-hierarchy.md)）
...
| **GCM-AES** | Galois/Counter Mode | 数据面加密+认证模式，tag 即 ICV（[cipher-suites.md](cipher-suites.md)） |
...
## 4. SecTAG 与帧字段（详见 [macsec-protocol-analysis.md](macsec-protocol-analysis.md)）
...
| **Peer List (Live/Potential)** | 对等体列表 | 类型 1/2：已互认/单向可见的成员（MI+MN 条目） |
...
| **PN exhaustion** | PN 耗尽 | 32 位 PN 用尽前必须换钥：100G 小帧约 29 秒（[lifecycle.md](lifecycle.md) §1） |
| **validate frames** | 帧校验策略 | strict/checked/disabled：ICV 挂与 untagged 帧的处理（[secy-processing.md](secy-processing.md) §4） |
...
## 6. XPN 专用（详见 [cipher-suites.md](cipher-suites.md) §3）
...
| **NIC offload** | 网卡卸载 | SecY 在网卡执行：线速 GCM、捕获点语义变化（[deployments.md](deployments.md)） |
| **`InPktsBadTag`** 等计数器 | — | 每 SA 丢弃计数：运维判断 MACsec 健康度的主依据（[secy-processing.md](secy-processing.md) §5） |

---

生成关系图（CAK→KEK/ICK→SAK）见 [key-hierarchy.md](key-hierarchy.md)；MKA 字段的逐字节位置见 [mka-reference.md](mka-reference.md)。
