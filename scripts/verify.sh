...
check 1/9 "reference pcaps exist" test -s captures/mka-handshake.pcap -a -s captures/session-full.pcap -a -s captures/mka-after-eap.pcap -a -s captures/mka-rekey.pcap -a -s captures/mka-co30.pcap
check 2/9 "keys.json present" test -s captures/keys.json
check 3/9 "decoded MKA report exists" test -s captures/decoded/01-mka-handshake.md -a -s captures/decoded/11-mka-after-eap.md -a -s captures/decoded/13-mka-rekey.md
...
if command -v tshark >/dev/null; then
    ...
    check 4/9 "tshark sees >=6 MKA frames (got ${mka})" test "${mka}" -ge 6
    check 5/9 "tshark sees >=6 MACsec frames (got ${mac})" test "${mac}" -ge 6
    check 6/9 "tshark sees IEEE encrypt vector (got ${ieee})" test "${ieee}" -ge 1
    ...
    check 7/9 "tshark sees EAP-Success then MKA (eap=${eap} mka=${eap_mka})" test "${eap}" -ge 1 -a "${eap_mka}" -ge 6
