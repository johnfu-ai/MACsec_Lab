## 5.4 数据面七帧：MACsec 逐帧解析

SAK 装好之后，用户流量开始走 SecY。本节拆解 [`session-full.pcap`](../captures/session-full.pcap) 的帧 7–13：三组加密的 ICMP echo 往返（PN 1–3），外加一帧 ES=1、省略 SCI 的特殊编码。看这七帧时，最值得盯住的是三件事：PN 如何递增、SCI 如何随方向切换、以及帧 13 凭什么能少 8 字节还验得过 ICV。
...
### 5.4.1 GCM 构造：密文与 ICV 从哪来

先复习 SecY 的密码学记号，与 NIST SP 800-38D / IEEE 测试向量一致：
...
User Data = 原帧的 EtherType + 载荷（不含外层 DA/SA）。实验室里是 `08 00` + IPv4 ICMP。offset 变体的展开见 [5.6 节](5.6_offset_and_vectors.md)。

下面七帧全部是 E=1,C=1、offset 0 的机密性+完整性模式：AAD 覆盖 DA‖SA‖SecTAG，明文只存在于发送端与接收端。
...
看点：第一条受保护流量。TCI/AN=`0x2c`（SC=1, E=1, C=1, AN=0），SCI 显式携带且是 **A 的** SCI——IV 由发送方 SCI 与 PN 拼出。
...
看点：方向反过来，SCI 换成 **B 的**——MACsec 的每个方向是一个独立 SC，各自从 PN=1 起步，互不干扰。
...
看点：同一 SC 上 PN 递增到 2——IV 随之变化，这就是"同一把 SAK 上 PN 不得重复"的直接原因。
...
看点：B 方向的 PN=2。两个方向的 PN 空间彼此独立，都从 1 开始。
...
看点：A 方向 PN=3。注意明文完全相同的三次 echo（只有 ICMP 序号与校验和不同），密文却毫无规律——GCM 是流密码式的，密钥流由 IV 决定。
...
看点：最后一帧"标准形态"的加密流量。B 方向 PN=3，与 A 方向的 PN 进度一致纯属实验节奏，两条 SC 并不共享序号空间。
...
看点：TCI/AN=`0x4c`——ES=1、SC=0，SecTAG 里**没有**那 8 字节 SCI，整帧因此只有 76 B。接收方从 `SA ‖ 00-01` 推回 SCI，GCM IV 仍用完整 8 字节。这是点对点链路上省带宽的常见编码。
...
至此 13 帧全部拆完。控制面建立信任、分发密钥；数据面用这把密钥把明文流量变成密文与 tag——书里此后的章节，都是在这条主线上展开的变奏。
