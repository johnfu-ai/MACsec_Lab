...
- `[captures/decoded/20-mka-delay-protect.md](captures/decoded/20-mka-delay-protect.md)` — Delay Protect：SAK Use 宣告 LLPN 下沿，被截留的帧延迟重放必弃（经典窗口会接受）
...
| `captures/macsec-replay.pcap`             | 接收端重放窗口四种裁决：按序 / 窗口内乱序接受、低于下沿 / 重复丢弃；重放帧 ICV 依然校验通过（[docs/lifecycle.md](docs/lifecycle.md) §3.1） |
| `captures/mka-delay-protect.pcap`         | Delay Protect：被截留的帧在 SAK Use 宣告 LLPN 下沿后重放必弃——经典窗口会接受的盲区（[docs/lifecycle.md](docs/lifecycle.md) §3.2） |
